...
interface LocationState {
  record?: Requirement & { advertisementId?: string; companyId?: string };
...
const CreateActions = () => {
  ...
  const advertisementId =
    state?.record?.advertisement_id ??
    state?.record?.advertisementId ??
...
interface GroupedPrefectureInputProps {
  source: string;
  choices?: Prefecture[];
  isLoading?: boolean;
  label?: string;
...
const GroupedPrefectureInput = ({
  ...
  const handleToggle = (id: string | number) => {
    if (selectedIds.includes(id)) {
      field.onChange(
        selectedIds.filter((itemId: string | number) => itemId !== id),
      );
    } else {
    ...
    >
      ...
      <div style={{ width: "100%" }}>
        {regions.map((region) => {
          ...
          const regionPrefs = choices.filter((d) =>
            region.prefs.includes(d.prefecture),
          );
              ...
              <FormGroup row>
                <Grid container spacing={1}>
                  {regionPrefs.map((pref) => (
                    // @ts-expect-error: Grid item prop issue with current MUI version
                    <Grid xs={6} sm={4} md={3} key={pref.id}>
                      <FormControlLabel
                        control={
                          <Checkbox
      ...
      <FormHelperText>{hasError ? error.message : helperText}</FormHelperText>
    </FormControl>
  );
};
...
export const RequirementCreate = () => {
  ...
  const advertisement_id =
    stateRecord?.advertisement_id ?? stateRecord?.advertisementId;
  const company_id =
  ...
  const { data: prefecturesData, isLoading: isLoadingPrefectures } =
    useGetList<Prefecture>("prefectures", {
    ...
    >
      <SimpleForm>
        <ReferenceInput
          ...
          reference="job_categories"
          label="職種"
        >
          <RadioButtonGroupInput
            optionText="job_category_name"
            label="職種"
        ...
        />
        <ReferenceArrayInput
          source="submission_objects_id"
          reference="submission_objects"
          label="提出物"
        >
          <CheckboxGroupInput
            optionText="submission_object_name"
            label="提出物"
            validate={validateRequired}
            helperText="複数選択可"
          />
        ...
        <NumberInput
          source="starting_salary_first"
          label="初任給 (1年卒)"
          placeholder="220000"
          helperText="半角英数字で入力してください　年次による差がない場合は入力なし"
        />
        <NumberInput
        ...
        />
        <NumberInput
          source="starting_salary_third"
          label="初任給 (3年卒)"
          placeholder="220000"
          helperText="半角英数字で入力してください　年次による差がない場合は入力なし"
        />
        <NumberInput
          source="starting_salary_fourth"
          label="初任給 (4年卒)"
          placeholder="220000"
          helperText="半角英数字で入力してください　年次による差がない場合は入力なし"
        />
        ...
        <ReferenceArrayInput
          source="welfare_benefits_id"
          reference="welfare_benefits"
          label="福利厚生"
        >
          <CheckboxGroupInput
            optionText="welfare_benefit"
            label="福利厚生"
            helperText="複数選択可"
          />
        </ReferenceArrayInput>
